...
const RenderTopIcon = ({ type, featured }) => {
  if (type === "single") {
    return (
...
export default function WebsitePlansSection() {
  return (
    <section className="relative isolate w-full overflow-hidden bg-[#050505] text-white">
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/2 z-[1] h-[550px] w-[550px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#f6a821]/[0.035] blur-[140px]" />
          ...
          <div>
            ...
            <h2 className="max-w-3xl text-4xl font-semibold leading-[0.92] tracking-[-0.055em] text-white sm:text-5xl md:text-6xl lg:text-[72px]">
             Professional Website 
              <br />
              <span className="text-[#f6a821]">Designs for Your Business</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-8 items-stretch">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`
                group relative flex flex-col justify-between overflow-hidden rounded-[30px] border p-8 backdrop-blur-2xl transition-all duration-500 hover:-translate-y-1.5
                ${
                  plan.featured
                    ? "border-[#f6a821]/45 bg-[#0e0e0e]/85 shadow-[0_25px_80px_rgba(246,168,33,0.12)] hover:border-[#f6a821]/60 hover:shadow-[0_30px_90px_rgba(246,168,33,0.18)]"
                    : "border-white/[0.08] bg-[#0b0b0b]/70 shadow-[0_20px_60px_rgba(0,0,0,0.35)] hover:border-[#f6a821]/30 hover:bg-[#101010]/80 hover:shadow-[0_25px_70px_rgba(0,0,0,0.45)]"
                }
              `}
            >
              {/* Gold Glass Reflection */}
              <div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-[#f6a821]/[0.04] blur-3xl transition-transform duration-700 group-hover:scale-125" />

              {/* Featured Badge */}
              {plan.featured && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                  <span className="inline-block rounded-full border border-[#f6a821]/50 bg-[#f6a821] px-4 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#050505] shadow-[0_4px_15px_rgba(246,168,33,0.3)]">
                    {plan.badgeText}
                  </span>
                </div>
              )}
              ...
              <div>
                ...
                <div className="mb-6 pt-1">
                  <RenderTopIcon type={plan.iconType} featured={plan.featured} />
                </div>
                ...
                <div className="my-6">
                  ...
                  <div className="mt-2 flex items-center gap-2 font-mono text-[11px] text-white/40">
                    <span>{plan.gst}</span>
                    <span>•</span>
                    <span className="font-semibold text-[#f6a821]/80">{plan.usd}</span>
                  </div>
                </div>
                ...
                <ul className="mb-8 space-y-3.5">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-[12px] leading-5 text-white/70">
              ...
              <Link
                href="/pages/contact"
                className={`
                  group/btn flex h-12 w-full items-center justify-between rounded-2xl px-5 text-xs font-semibold transition-all duration-300
                  ${
                    plan.featured
                      ? "bg-[#f6a821] text-[#050505] shadow-[0_10px_30px_rgba(246,168,33,0.25)] hover:bg-[#ffb638] hover:shadow-[0_12px_35px_rgba(246,168,33,0.35)]"
                      : "border border-white/[0.12] bg-white/[0.04] text-white hover:border-[#f6a821]/40 hover:bg-white/[0.08] hover:text-[#f6a821]"
                  }
                `}
              >
